...
namespace KayceesModModifierAPI
{
    ...
    public class AscensionChooseStarterDeckScreen_Start
    {
        [HarmonyPostfix]
        public static void Postfix(AscensionChooseStarterDeckScreen __instance)
        {
            if(__instance.GetComponent<AscensionDeckSelectPageManager>() == null)
            {
                List<StarterDeckInfo> decksToAdd = new List<StarterDeckInfo>(KayceeModModifier.CustomAddedDecks.ConvertAll((CustomStarterDeckInfo inf) => inf.ToStarterDeck()));
                ...
                {
                    AscensionDeckSelectPageManager manager = __instance.gameObject.AddComponent<AscensionDeckSelectPageManager>();
